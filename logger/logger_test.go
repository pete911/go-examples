...
func TestLog(t *testing.T) {
	...
	match, err := regexp.MatchString(infoMsgRegex, msg)
	if !match {
		t.Errorf("%s does not match %s", msg, infoMsgRegex)
	}
	if err != nil {
...
func TestErrLog(t *testing.T) {
	...
	match, err := regexp.MatchString(errMsgRegex, msg)
	if !match {
		t.Errorf("%s does not match %s", msg, errMsgRegex)
	}
	if err != nil {
		t.Errorf("unexpected error: %v", err)
	}
